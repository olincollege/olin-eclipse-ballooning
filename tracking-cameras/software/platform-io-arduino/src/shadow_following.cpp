...
#include <SPI.h>
#include <calculate_shadow_angles.h>
...
void computeSunPos()
{
    ...
    // int hour = sttime.hour;
    // int minute = sttime.minute;
    // int second = sttime.second;

    // int hours = 16;
    // int minutes = 20;
    // int seconds = 15;

    // // float balloonLatitude = loc.latitude;
    // // float balloonLongitude = loc.longitude;
    // // float balloonHeight = hMSL;

    // float balloonLatitude = 30.511833;
    // float balloonLongitude = -99.761336;
    // float balloonHeight = 30000.0;

    // std::pair<double, double> trackingAngles = ShadowTrack(hours, minutes, seconds, balloonLatitude, balloonLongitude, balloonHeight);

    // azimuth = trackingAngles.first;
    // altitude = -trackingAngles.second;
...
void adjustCameraPanAngle()
{
    ...
    Serial.print("Azimuth of the shadow: ");
    Serial.println(azimuth);
    Serial.print("Altitude of the shadow: ");
    Serial.println(altitude);
    Serial.print("Compass yaw: ");
    Serial.println(convertTo360(ypr.yaw));
    Serial.print("Pan motor rotation: ");
    Serial.println(currentPanPos);
...
void adjustCameraTiltAngle()
{
    ...
    float rollDegrees = convertTo360(ypr.roll); // also flips the positive axis
    if (rollDegrees > 180.0)
    {
        rollDegrees -= 360.0;
    }

    float pitchDegrees = 360 - convertTo360(ypr.pitch);
    ...
    Serial.print("Tilt motor direction: ");
    Serial.println(currentTiltPos);
    Serial.println("");
    ...
    if (abs(tiltDiff) > TILT_TOLERANCE)
    {
        ...
        steps = max(steps, tilt_angle_to_steps(5 - currentTiltPos));
